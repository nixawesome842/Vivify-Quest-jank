...
static int id = UnityEngine::Shader::PropertyToID(u”*StereoActiveEye”);
return id;
}
...
static int id = UnityEngine::Shader::PropertyToID(u”*UnityStereoEyeIndex”);
return id;
}
...
enabled && stereoMode.value** != UnityEngine::XR::XRSettings_StereoRenderingMode::MultiPass.value__);
...
if (ShouldHideOriginal(validModelInfos)) {
  // Use parent->get_gameObject() so we search the actual saber visual
  // hierarchy (the transform passed into SaberModelController::Init),
  // not just the SMC's own GameObject which may have no renderers as
  // children.
  auto* parentGO = IsAlive(parent) ? parent->get_gameObject().unsafePtr() : nullptr;
  if (IsAlive(parentGO)) {
    DisableOriginalRenderers(parentGO, replacement);
  }
  // Also disable any renderers directly on the SMC GameObject itself.
  if (IsAlive(smc->get_gameObject())) {
...
// Don’t gate on mask == 0 here. GameplayOverlayLayerMask() always ORs in
// the static note/saber/debris layer masks, so it is never truly zero
// during gameplay. Destroying the camera when _gameplayOverlayLayerMask
// happens to be zero (e.g. right after RestoreOverlayRenderState clears
// it) was causing pooled notes — whose Init hooks don’t re-fire — to
// never get their renderers registered again, leaving them invisible.
if (!allowOverlay || !IsAlive(mainCam) || !IsAlive(mainCamGO)) {
DestroyGameplayOverlayCamera();
return;
}
...
bool const needsBlit = !GetDisableAllBlits() && (!*preEffects.empty() || !*postEffects.empty());
if (*cameraApplier->get_enabled() != needsBlit) {
*cameraApplier->set_enabled(needsBlit);
}
}
void DestroyCameraApplier() {
if (*cameraApplier != nullptr && UnityEngine::Object::op_Implicit_bool(*cameraApplier)) {
*cameraApplier->set_enabled(false);
UnityEngine::Object::Destroy(*cameraApplier);
}
*cameraApplier = nullptr;
}
void HandleLevelSelected(SongCore::API::LevelSelect::LevelWasSelectedEventArgs const& event) {
ResetRuntime();
*selectedLevelPath.clear();
*selectedMapHasVivifyRequirement = false;
if (!event.isCustom || event.customBeatmapLevel == nullptr) {
SongCore::API::PlayButton::EnablePlayButton(“Vivify”);
return;
}
*selectedLevelPath = std::string(event.customBeatmapLevel->customLevelPath);
if (GetVivifyDebugLogging()) {
PaperLogger.info(“Vivify level selected: path=’{}’ isCustom={} hasDetails={}”,
*selectedLevelPath, BoolText(event.isCustom), BoolText(event.customLevelDetails.has_value()));
}
if (event.customLevelDetails) {
auto const& requirements = event.customLevelDetails->difficultyDetails.requirements;
*selectedMapHasVivifyRequirement = std::any_of(requirements.begin(), requirements.end(), [](std::string const& requirement) {
return requirement == kCapability;
});
}
if (*selectedMapHasVivifyRequirement) {
MetaCore::Game::SetScoreSubmission(“Vivify”, false);
std::string bundlePath = JoinPath(*selectedLevelPath, kBundleFile);
bool bundleExists = std::filesystem::exists(bundlePath);
if (!bundleExists) {
std::string lowerBundlePath = JoinPath(*selectedLevelPath, “bundleandroid2021.vivify”);
...
std::string infoPath = JoinPath(*selectedLevelPath, “Info.dat”);
if (!std::filesystem::exists(infoPath)) infoPath = JoinPath(*selectedLevelPath, “info.dat”);
...
if (doc.HasMember(”*customData”)) customData = &doc[”*customData”];
...
if (customData->HasMember(”*assetBundle”)) assetBundle = &(*customData)[”_assetBundle”];
...
if (*beatmapAD != nullptr) {
return *beatmapAD->getPointDefinition(object, key, type);
}
if (!value->IsArray()) {
return std::nullopt;
}
return PointDefinitionW(*value, type, *fallbackBeatmapAD.GetBaseProviderContext());
}
...
if (!GetVivifyDebugLogging() || *loggedUnityPlatformInfo) return;
*loggedUnityPlatformInfo = true;
auto stereoMode = UnityEngine::XR::XRSettings::get_stereoRenderingMode();
...
context, requested.value**, fallback.value**);
}
return fallback;
}
...
*gameplayOverlayLayerMask |= static_cast<int>(1u << static_cast<unsigned>(layer));
}
...
if (!*overlayRendererSortingOrders.contains(renderer)) {
*overlayRendererSortingOrders.emplace(renderer, renderer->get_sortingOrder());
}
...
for (auto const& [renderer, sortingOrder] : *overlayRendererSortingOrders) {
if (IsAlive(renderer)) {
renderer->set_sortingOrder(sortingOrder);
}
}
*overlayRendererSortingOrders.clear();
*gameplayOverlayLayerMask = 0;
}
...
left.get_graphicsFormat().value** == right.get_graphicsFormat().value** &&
left.get_depthStencilFormat().value** == right.get_depthStencilFormat().value** &&
left.get_dimension().value** == right.get_dimension().value**;
}
...
ReleaseRenderTexture(*mainBlitTexture);
ReleaseRenderTexture(*scratchBlitTexture);
}
...
bool const enabled = GetMultipassRenderingEnabled();
auto stereoMode = UnityEngine::XR::XRSettings::get_stereoRenderingMode();
bool const singlePass = enabled && stereoMode.value** != UnityEngine::XR::XRSettings_StereoRenderingMode::MultiPass.value**;
bool const instanced = enabled && stereoMode.value** == UnityEngine::XR::XRSettings_StereoRenderingMode::SinglePassInstanced.value**;
bool const multiview = enabled && stereoMode.value** == UnityEngine::XR::XRSettings_StereoRenderingMode::SinglePassMultiview.value**;
bool const doubleWide = enabled && stereoMode.value** == UnityEngine::XR::XRSettings_StereoRenderingMode::SinglePass.value**;
...
*repairedMaterials.emplace(material);
}
...
return smc->****colorManager->ColorForSaberType(saber->get_saberType());
}
return saber != nullptr && saber->get_saberType().value** == GlobalNamespace::SaberType::SaberA.value**
...
*noteReplacements.clear();
for (auto& [*, replacement] : _saberReplacements) RestoreReplacementData(replacement);
*saberReplacements.clear();
for (auto& [*, replacement] : _debrisReplacements) RestoreReplacementData(replacement);
...
// Two InstantiatePrefab events share the same id. The second event
// would overwrite *livePrefabs[id] when it fires, destroying whatever
// the first event placed there. Keep them as separate preload entries
// (keyed by event pointer) so each event manages its own instance;
// GetPrefabStorageId falls back to the pointer-based key when id is
// cleared.
data.id = std::nullopt;
}
}
...
CleanCustomObject(data.instance);
RepairGameObjectMaterials(data.instance, data.asset);
...
return “vivify_prefab*” + std::to_string(reinterpret_cast<std::uintptr_t>(customEventData));
}
...
// DestroyPrefabById above may have destroyed data.instance if the live
// prefab and the preloaded instance were the same object (which happens
// when the same event fires a second time, e.g. in a looping map or when
// two events share an id). Re-instantiate from the source prefab so we
// always hand a fresh, valid object to the scene.
if (data.instance == nullptr || !UnityEngine::Object::op_Implicit_bool(data.instance)) {
auto* prefab = GetAssetAs<UnityEngine::GameObject>(data.asset);
...
float *songTimeCache = 0.0f;
bool *loggedUnityPlatformInfo = false;
...
AssignedPrefabKind kind = noteData->get_cutDirection().value** == GlobalNamespace::NoteCutDirection::Any.value**
